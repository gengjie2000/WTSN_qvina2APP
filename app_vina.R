...
ui <- fluidPage(
  ...
  titlePanel("AutoDock Vina 分子对接平台"),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      # Vina路径设置（可选）
      textInput("vina_path", "Vina路径 (可选):", 
                value = "",
                placeholder = "留空则使用系统PATH中的vina"),
      helpText("如果不确定，请留空。系统会自动查找Vina。"),
      ...
      # CPU核心数（Vina特有参数）
      numericInput("cpu", "CPU核心数:", value = 1, min = 1, max = 32),
      helpText("设置使用的CPU核心数，0表示使用所有核心"),
      ...
      tabsetPanel(
        ...
        tabPanel("帮助文档",
                 ...
                 tags$ul(
                   ...
                   tags$li("自动修复功能会调整PDBQT文件的格式以满足Vina的要求"),
                   tags$li("OpenBabel通常能更好地处理小分子格式转换"),
                   tags$li("如果SMILES文件转换失败，请勾选'生成3D坐标'"),
                   tags$li("Vina支持多CPU核心并行计算，可设置CPU核心数提高速度")
                 )
        )
      )
    )
...
server <- function(input, output, session) {
  ...
  # 显示Vina状态
  output$vina_status <- renderUI({
    if (input$vina_path != "") {
      # 如果用户指定了路径，检查该路径
      if (file.exists(input$vina_path)) {
        # 测试文件是否可执行
        test_cmd <- paste(shQuote(input$vina_path), "--help")
        tryCatch({
          result <- system(test_cmd, intern = TRUE, ignore.stderr = TRUE, timeout = 2)
          if (length(result) > 0) {
            values$vina_found <- TRUE
            values$vina_path <- input$vina_path
            return(tags$div(class = "alert alert-success",
                            icon("check-circle"), " Vina路径有效: ", input$vina_path))
          } else {
            values$vina_found <- FALSE
            return(tags$div(class = "alert alert-warning",
                            icon("exclamation-triangle"), " 无法验证Vina程序"))
          }
        }, error = function(e) {
          values$vina_found <- FALSE
          return(tags$div(class = "alert alert-warning",
                          icon("exclamation-triangle"), " Vina验证失败: ", e$message))
        })
      } else {
        values$vina_found <- FALSE
        return(tags$div(class = "alert alert-danger",
                        icon("exclamation-triangle"), " Vina未找到: ", input$vina_path))
      }
    } else {
      # 如果用户未指定路径，尝试在系统PATH中查找
      tryCatch({
        # 方法1: 尝试直接运行vina --help
        result <- system("vina --help", intern = TRUE, ignore.stderr = TRUE, timeout = 2)
        if (length(result) > 0) {
          values$vina_found <- TRUE
          values$vina_path <- "vina"
          return(tags$div(class = "alert alert-success",
                          icon("check-circle"), " Vina已找到 (系统PATH)"))
        } else {
          # 方法2: 尝试不同命令变体（Windows/Linux兼容）
          commands <- c(
            "vina.exe", "vina", "Vina.exe", "Vina",
            "autodock_vina.exe", "autodock_vina"
          )
          ...
          for (cmd in commands) {
            tryCatch({
              ...
              if (length(test_result) > 0) {
                values$vina_found <- TRUE
                values$vina_path <- cmd
                return(tags$div(class = "alert alert-success",
                                icon("check-circle"), " Vina已找到 (命令:", cmd, ")"))
              }
            }, error = function(e) {
              # 继续尝试下一个命令
            })
          ...
          for (path in common_paths) {
            if (file.exists(path)) {
              values$vina_found <- TRUE
              values$vina_path <- path
              return(tags$div(class = "alert alert-success",
                              icon("check-circle"), " Vina已找到: ", path))
            }
          }
          ...
          return(tags$div(class = "alert alert-warning",
                          ...
                          " 未在系统PATH中找到Vina。请在左侧输入完整路径，或确保已添加到系统PATH。"))
        }
      }, error = function(e) {
        values$vina_found <- FALSE
        return(tags$div(class = "alert alert-warning",
  ...
  check_vina <- function() {
    ...
    if (input$vina_path != "") {
      # 使用用户指定的路径
      vina_path <- input$vina_path
      if (!file.exists(vina_path)) {
        add_log(paste("错误: 指定的Vina路径不存在:", vina_path))
        return(FALSE)
      }
      
      # 测试Vina
      test_cmd <- paste(shQuote(vina_path), "--help")
      tryCatch({
        result <- system(test_cmd, intern = TRUE, ignore.stderr = TRUE, timeout = 5)
        if (length(result) > 0) {
          add_log(paste("✓ Vina检测成功:", vina_path))
          values$vina_path <- vina_path
          return(TRUE)
        } else {
          add_log("⚠ Vina可能不是有效版本")
          return(FALSE)
        }
      }, error = function(e) {
        add_log(paste("✗ 无法运行Vina:", e$message))
        return(FALSE)
      })
    } else {
      # 尝试在系统PATH中查找
      tryCatch({
        # 方法1: 直接运行vina --help
        result <- system("vina --help", intern = TRUE, ignore.stderr = TRUE, timeout = 5)
        if (length(result) > 0) {
          add_log("✓ Vina检测成功 (系统PATH)")
          values$vina_path <- "vina"
          return(TRUE)
        } else {
          # 方法2: 尝试不同命令变体
          commands <- c("vina.exe", "vina", "Vina.exe", "Vina", "autodock_vina.exe", "autodock_vina")
          ...
          add_log("✗ 未在系统PATH中找到Vina")
          add_log("请确保Vina已添加到系统PATH，或在左侧输入完整路径")
          return(FALSE)
        }
      }, error = function(e) {
        add_log(paste("✗ 检查Vina时出错:", e$message))
        return(FALSE)
      })
    }
  }
  ...
  create_config_file <- function(protein_file, ligand_file, output_dir, ligand_name) {
    ...
    config_content <- paste(
      "# AutoDock Vina configuration file for ligand:", ligand_name,
      ...
      "\n\n# CPU cores to use (0 means all available cores)",
      "\ncpu = ", ifelse(input$cpu == 0, 0, input$cpu),
      sep = ""
    )
  ...
  run_docking <- function(config_file, ligand_name, output_dir, ligand_pdbqt, index, total) {
    ...
    # 构建Vina对接命令
    vina_cmd <- values$vina_path
    
    # 构建完整命令 - 使用标准Vina命令行参数
    cmd <- paste(
      shQuote(vina_cmd),
    ...
    tryCatch({
      ...
      # 检查是否有错误信息
      error_lines <- result[grepl("error|Error|ERROR|Parse error|failed|Failed", result, ignore.case = TRUE)]
      if (length(error_lines) > 0) {
        add_log(paste("Vina错误信息:", paste(error_lines, collapse = "\n")))
      }
      
      # 检查命令执行状态
      status <- attr(result, "status")
      if (!is.null(status) && status != 0) {
        add_log(paste("Vina执行失败，错误码:", status))
        return(list(success = FALSE, energy = NA, all_energies = numeric(0), message = paste("错误码:", status)))
      }
      
      # 从日志文件中提取结合能（Vina的标准输出格式）
      if (file.exists(log_file)) {
        tryCatch({
          ...
          for (line in log_content) {
            ...
            # 另一种格式: "Affinity: -8.1 kcal/mol"
            if (grepl("Affinity:\\s*-?\\d+\\.\\d+\\s*kcal/mol", line, ignore.case = TRUE)) {
              energy_match <- regmatches(line, regexpr("-?\\d+\\.\\d+", line))
              if (length(energy_match) > 0) {
                energy <- as.numeric(energy_match)
                energies <- c(energies, energy)
              }
            }
          }
      ...
      if (file.exists(out_file)) {
        ...
        tryCatch({
          ...
          for (line in out_content) {
            if (grepl("REMARK VINA RESULT:", line)) {
              # 格式: "REMARK VINA RESULT:    -8.1      0.000      0.000"
              parts <- strsplit(trimws(line), "\\s+")[[1]]
              if (length(parts) >= 4 && grepl("-?\\d+\\.\\d+", parts[3])) {
                energy <- as.numeric(parts[3])
                add_log(paste("对接成功! 从输出文件中提取到结合能:", energy, "kcal/mol"))
                return(list(success = TRUE, energy = energy, all_energies = c(energy), message = "成功"))
              }
            }
          }
        }, catch = function(e) {})
  ...
  run_docking_pipeline <- function() {
    ...
    # 检查Vina是否可用
    add_log("检查Vina是否可用...")
    update_progress(0, 0, 0, 0, "检查Vina...", "")
    
    if (!check_vina()) {
      add_log("错误: Vina未找到或未正确安装")
      add_log("请在左侧输入Vina的完整路径，或确保已添加到系统PATH")
      return()
    }
  ...
  output$protein_info <- renderText({
    if (!is.null(input$protein_file)) {
      paste(
        ...
        "CPU核心数:", ifelse(input$cpu == 0, "全部", input$cpu)
      )
    } else {
      "请先上传蛋白质文件"
    }
